...
// Also called by cron: Saturday 22:30 UTC weekly schedule

const supabaseAdmin = require('../lib/supabaseAdmin');
const { measureInfluencerMentions } = require('../lib/aiMeasurement');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    for (const client of clients) {
      ...
      const clientCitationRows = [];
      const clientGeoFlags = {};
      ...
      for (const influencer of influencers) {
        ...
        const engineMap = {
          ...
          google_aio: 'google_aio', gemini: 'gemini', claude: 'claude'
        };
        ...
        for (const [engineKey, engineName] of Object.entries(engineMap)) {
          ...
          if (engineResult.mentioned || (engineResult.urls && engineResult.urls.length > 0)) {
            ...
            const urlsToRecord = (engineResult.urls && engineResult.urls.length > 0)
              ? engineResult.urls.slice(0, 3)
              : ['(text mention)'];
            for (const citedUrl of urlsToRecord) {
              clientCitationRows.push({
                ...
                cited_url: citedUrl,
                citation_text: engineResult.excerpt || null,
        ...
        await supabaseAdmin.from('geo_scores').upsert(
          {
            ...
            scored_at: today,
            geo_score: geoScore,
            cited_by_chatgpt: !!geoFlags.cited_by_chatgpt,
            cited_by_perplexity: !!geoFlags.cited_by_perplexity,
            cited_by_google_aio: !!geoFlags.cited_by_google_aio,
            cited_by_gemini: !!geoFlags.cited_by_gemini,
            cited_by_claude: !!geoFlags.cited_by_claude,
          },
          { onConflict: 'influencer_id,scored_at' }
        );
        await supabaseAdmin.from('influencers')
          .update({ geo_score: geoScore, geo_scored_at: today })
          .eq('id', influencer.id);
      }
      ...
      results.push({
        client_id: client.id,
        name: client.name,
        influencers_scanned: influencers.length,
        citations_recorded: clientCitationRows.length,
        geo_flags_set: Object.keys(clientGeoFlags).length,
      });
