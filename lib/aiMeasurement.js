...
async function measureWithChatGPT(brandNames, industry) {
  ...
  const results = await Promise.all(queries.map(async (query) => {
    try {
      ...
      return { query, response, mentioned, citations };
    } catch (err) {
  ...
  return { score, mentionCount, totalQueries: queries.length, details: results, citations: allChatGPTCitations };
}
...
async function measureWithPerplexity(brandNames, industry) {
  ...
  return { score, mentionCount, totalQueries: queries.length, details: results, citations: allPerplexityCitations };
}
...
async function measureWithClaude(brandNames, industry) {
  ...
  return { score, mentionCount, totalQueries: queries.length, details: results, citations: allCitations };
}
...
async function measureWithGemini(brandNames, industry) {
  ...
  for (const query of queries) {
    ...
    try {
      ...
      const mentioned = checkMention(response, brandNames);
      const citations = extractUrlsFromText(response);
      results.push({ query, response, mentioned, citations });
    } catch (err) {
  ...
  return { score, mentionCount, totalQueries: validTotal, details: results, citations: allGeminiCitations };
}
