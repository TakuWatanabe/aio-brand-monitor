 ...
 *
 * @param {Array<{name, cat, handle?, followers?, impact?}>} influencers  - 登録済みインフルエンサー一覧
 * @param {string[]} brandNames  - 計測対象ブランド名
 * @param {string}   industry    - 業種（クエリの文脈に使用）
 * @returns {Promise<Array>}     - impact フィールドが更新されたインフルエンサー配列
 *
 * スコア式: SNS言及確認数 × フォロワー影響力倍率
 *   100万人超 ×10 / 50万 ×7 / 10万 ×5 / 5万 ×3 / 1万 ×2 / 1万未満 ×1
 */
async function measureInfluencerMentions(influencers, brandNames, industry) {
  ...
  function followerMultiplier(followers) {
    ...
    if (f >= 1_000_000) return 10;
    if (f >=   500_000) return 7;
    if (f >=   100_000) return 5;
    if (f >=    50_000) return 3;
    if (f >=    10_000) return 2;
    return 1;
  }

  // ブランドのSNS言及状況を把握するグローバルクエリ
  const globalQueries = [
    `${brand}をSNS（Instagram・X・TikTok・YouTube）で紹介・推薦しているインフルエンサーを教えてください`,
    `${brand}のインフルエンサーマーケティング・PR投稿やコラボ事例をSNSで教えてください`,
    `${brand}についてSNSで最近言及・投稿しているインフルエンサーや有名人は誰ですか？`,
    `${industry}業界でSNSインフルエンサーを活用しているブランドの事例と代表的なインフルエンサーを教えてください`,
  ];
  ...
  try {
    ...
    if (perplexityKey) {
      // Perplexityはウェブ検索があるため、インフルエンサー個別クエリも追加（上位5名）
      const perplexityQueries = [
        ...globalQueries,
        ...influencers.slice(0, 5).map(inf => {
          const nameOrHandle = inf.handle ? `${inf.name} (${inf.handle})` : inf.name;
          return `${nameOrHandle} ${brand} SNS 投稿 紹介 最近`;
        }),
      ];
      const responses = await Promise.all(
        perplexityQueries.map(q =>
  ...
  const updated = influencers.map(inf => {
    ...
    // フォロワー倍率を掛けてSNS影響力スコアを算出
    const multiplier = followerMultiplier(inf.followers);
    const oldImpact  = inf.impact || 0;
    const newImpact  = mentionCount * multiplier;

    console.log(`[INF] ${inf.name}: 言及${mentionCount}件 × フォロワー倍率×${multiplier} = スコア${newImpact} (前回: ${oldImpact})`);
...
/**
 * インフルエンサー施策カテゴリ別にSNS言及スコアを集計し barData を生成する
 *
